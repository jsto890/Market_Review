...
if [ "$FETCH_RC" -ne 0 ]; then
  echo "X fetch failed (rc=$FETCH_RC) — trying free fallback sources"
  set +e
  PYTHONPATH=src "$PYTHON" -m stock_chatter.cli fetch-fallback --tickers "$FIXED_TICKERS"
  FALLBACK_RC=$?
  set -euo pipefail
  if [ "$FALLBACK_RC" -ne 0 ]; then
    echo "Fallback sources also failed — continuing with cached posts (sentiment STALE)"
    MEMO_FLAGS="--x-skipped"
  fi
fi
...
PYTHONPATH=src "$PYTHON" -m stock_chatter.cli memo --since-last ${MEMO_FLAGS:+$MEMO_FLAGS}
...
from stock_chatter.fallback_sources import fetch_stocktwits_trending_posts
...
try:
    ...
    posts = fetch_stocktwits_trending_posts(max_symbols=15)
